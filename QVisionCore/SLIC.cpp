...
#if 0 // chammoru disable: remove black contour
		for( int n = 0; n < 8; n++ )
		{
	...
	const double&				M)
{
	...
	for( int itr = 0; itr < 10; itr++ )
	{
		...
		{
			int kseedsyi = int(kseedsy[n]);
			int kseedsxi = int(kseedsx[n]);
            y1 = max(0,			kseedsyi-offset);
            y2 = min(m_height,	kseedsyi+offset);
            x1 = max(0,			kseedsxi-offset);
            x2 = min(m_width,	kseedsxi+offset);
	...
	const double&				compactness)
{
	...
	for( int itr = 0; itr < 5; itr++ )
	{
		...
		for( int n = 0; n < numk; n++ )
		{
			int kseedsyi = int(kseedsy[n]);
			int kseedsxi = int(kseedsx[n]);
			int kseedszi = int(kseedsz[n]);
            y1 = max(0,			kseedsyi-offset);
            y2 = min(m_height,	kseedsyi+offset);
            x1 = max(0,			kseedsxi-offset);
            x2 = min(m_width,	kseedsxi+offset);
            z1 = max(0,			kseedszi-offset);
            z2 = min(m_depth,	kseedszi+offset);
	...
	bool                        labInput)
{
    ...
    //--------------------------------------------------
    if (!labInput) // RGB input, the default option
    {
    ...
    else
    {
        ...
        for( int i = 0; i < sz; i++ )
        {
                m_lvec[i] = ubuff[3*i + 2];
                m_avec[i] = ubuff[3*i + 1];
                m_bvec[i] = ubuff[3*i];
        }
    }
	//--------------------------------------------------
	...
	EnforceLabelConnectivity(klabels, m_width, m_height, nlabels, numlabels, sz/(STEP*STEP));
	...
	bool                        labInput)
{
    ...
    DoSuperpixelSegmentation_ForGivenSuperpixelSize(ubuff,width,height,klabels,numlabels,superpixelsize,compactness, labInput);
}
